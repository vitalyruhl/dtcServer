...
namespace coinbase_dtc_core
{
    namespace server
    {
              ...
              connection_time_(Server::get_current_timestamp())
        {
            ...
            open_dtc_server::open_dtc_server::util::log("[CONN] New client session created from " + remote_addr_);
        }
        ...
        ClientSession::~ClientSession()
        {
            ...
            open_dtc_server::open_dtc_server::util::log("Client session destroyed: " + username_ + " (" + remote_addr_ + ")");
        }
        ...
        {
            if (socket_ == -1 || data.empty())
                return false;

            open_dtc_server::open_dtc_server::util::log("[SEND] Sending " + std::to_string(data.size()) + " bytes to " + remote_addr_);
            ...
            while (total_sent < data.size())
            {
                ...
                if (sent == -1)
                {
                    open_dtc_server::util::log("[ERROR] Send failed for client " + remote_addr_);
                    return false;
                }
                total_sent += sent;
            }

            open_dtc_server::util::log("[OK] Sent " + std::to_string(total_sent) + " bytes to " + remote_addr_);
            return true;
        }
              ...
              symbol_manager_(std::make_unique<SymbolManager>())
        {
            ...
            open_dtc_server::util::log("DTC Server initialized on port " + std::to_string(config_.port));
            open_dtc_server::util::log("[SYMBOL] Initialized with " + std::to_string(symbol_manager_->get_symbol_count()) + " symbols");
        ...
        void Server::initialize_websocket_client()
        {
            ...
            {
                open_dtc_server::util::log("[SERVER] Added Coinbase exchange successfully");
            }
            else
            {
                open_dtc_server::util::log("[SERVER] Warning: Failed to add Coinbase exchange");
            }

            open_dtc_server::util::log("[SERVER] Multi-exchange feed initialized - ready for dynamic subscriptions");
        }
        ...
        bool Server::start()
        {
            ...
            if (!init_socket())
            {
                open_dtc_server::util::log("Failed to initialize server socket");
                return false;
            }
        ...
        bool Server::set_socket_options(int socket)
        {
            ...
            {
                open_dtc_server::util::log("Failed to set SO_REUSEADDR");
                return false;
            }
                           ...
                           reinterpret_cast<char *>(&opt), sizeof(opt)) == -1)
            {
                ...
                open_dtc_server::util::log("Warning: Failed to set SO_REUSEPORT");
            }
#endif
        ...
        {
            open_dtc_server::util::log("Accept loop started");
            ...
            while (running_ && !shutdown_requested_)
            {
                ...
                {
                    open_dtc_server::util::log("Client limit reached, rejecting connection");
                ...
                open_dtc_server::util::log("Accepted connection from " + remote_addr);
            ...
            open_dtc_server::util::log("Accept loop ended");
        }

        void Server::handle_client(std::shared_ptr<ClientSession> client)
        {
            open_dtc_server::util::log("Client handler started for " + client->get_remote_address());
            ...
            open_dtc_server::util::log("Client handler ended for " + client->get_remote_address());
        }
        ...
        bool Server::process_client_messages(std::shared_ptr<ClientSession> client)
        {
            ...
            if (bytes_received == -1)
            {
                open_dtc_server::util::log("[ERROR] Receive failed for client " + client->get_remote_address());
                return false;
            }

            if (bytes_received == 0)
            {
                open_dtc_server::util::log("[DISC] Client disconnected: " + client->get_remote_address());
                return false;
            }

            // Debug: Log received data
            open_dtc_server::util::log("[RECV] Received " + std::to_string(bytes_received) + " bytes from " + client->get_remote_address());

            // Parse DTC messages
            auto message = protocol_.parse_message(buffer.data(), static_cast<uint16_t>(bytes_received));
            if (message)
            {
                open_dtc_server::util::log("[PARSE] Successfully parsed message type: " + std::to_string(static_cast<int>(message->get_type())));
                ...
                else
                {
                    ...
                    case dtc::MessageType::MARKET_DATA_REQUEST:
                        ...
                        break;
                    default:
                        open_dtc_server::util::log("[WARNING] Unhandled message type: " + std::to_string(static_cast<int>(type)));
                        break;
                    }
                }
            }
            ...
            {
                open_dtc_server::util::log("[ERROR] Failed to parse message from " + client->get_remote_address() +
                ...
                open_dtc_server::util::log(hex_dump);
            }
        ...
        void Server::handle_logon_request(std::shared_ptr<ClientSession> client, std::unique_ptr<dtc::DTCMessage> message)
        {
            ...
            open_dtc_server::util::log("[AUTH] Logon request from " + client->get_remote_address() + " - Username: '" + username + "'");
            ...
            if (auth_success)
            {
                ...
                open_dtc_server::util::log("[OK] Authentication successful for user: " + username);
            }
            else
            {
                open_dtc_server::util::log("[ERROR] Authentication failed for user: " + username);
            }
            ...
            open_dtc_server::util::log("[SEND] Sending logon response to " + username + " - Success: " + (auth_success ? "Yes" : "No"));

            if (!client->send_message(*response))
            {
                open_dtc_server::util::log("[ERROR] Failed to send logon response to " + client->get_remote_address());
            }
        }
        ...
        void Server::handle_market_data_request(std::shared_ptr<ClientSession> client, std::unique_ptr<dtc::DTCMessage> message)
        {
            ...
            {
                open_dtc_server::util::log("[ERROR] Market data request from unauthenticated client: " + client->get_remote_address());
                return;
            }

            auto *md_req = static_cast<dtc::MarketDataRequest *>(message.get());

            open_dtc_server::util::log("[MARKET] Market data request for symbol: '" + std::string(md_req->symbol) +
                      "' (action: " + std::to_string(md_req->request_action) +
                      ") from user: " + client->get_username());

            if (md_req->request_action == 1)
            { // Subscribe
                // Find symbol info
                auto symbol_info = symbol_manager_->get_symbol_by_name(md_req->symbol);
                if (!symbol_info)
                {
                    open_dtc_server::util::log("[ERROR] Unknown symbol: " + std::string(md_req->symbol));
                    return;
                }
                ...
                if (success)
                {
                    ...
                    open_dtc_server::util::log("[OK] Client " + client->get_username() + " subscribed to " +
                              symbol_info->symbol + " (ID: " + std::to_string(symbol_info->symbol_id) + ")");
                }
            }
            else if (md_req->request_action == 2)
            { // Unsubscribe
                // Find symbol info
                auto symbol_info = symbol_manager_->get_symbol_by_name(md_req->symbol);
                if (symbol_info)
                {
                    ...
                    if (success)
                    {
                        ...
                        open_dtc_server::util::log("[OK] Client " + client->get_username() + " unsubscribed from " +
                                  symbol_info->symbol + " (ID: " + std::to_string(symbol_info->symbol_id) + ")");
                    }
                }
            }
        ...
        void Server::on_trade_data(const feed::coinbase::TradeData &trade)
        {
            ...
            open_dtc_server::util::log("[TRADE] " + trade.product_id + " @ " +
        ...
        bool ClientSession::subscribe_symbol(uint32_t symbol_id, const std::string &symbol_name)
        {
            ...
            open_dtc_server::util::log("[SUB] Client " + username_ + " subscribed to " + symbol_name + " (ID: " + std::to_string(symbol_id) + ")");
            return true;
        }
        ...
        bool ClientSession::unsubscribe_symbol(uint32_t symbol_id)
        {
            ...
            open_dtc_server::util::log("[UNSUB] Client " + username_ + " unsubscribed from " + symbol_name + " (ID: " + std::to_string(symbol_id) + ")");
            return true;
        }
        ...
        void Server::update_websocket_subscriptions()
        {
            ...
            // Update active symbols if changed
            {
                ...
                if (needed_symbols != active_websocket_symbols_)
                {
                    ...
                    for (const auto &symbol : needed_symbols)
                    {
                        if (active_websocket_symbols_.find(symbol) == active_websocket_symbols_.end())
                        {
                            ...
                            if (multi_exchange_feed_)
                            {
                                ...
                                open_dtc_server::util::log("[SERVER] Added subscription for " + symbol + " on all exchanges");
                            }
                        }
                    }
                    ...
                    for (const auto &symbol : active_websocket_symbols_)
                    {
                        if (needed_symbols.find(symbol) == needed_symbols.end())
                        {
                            ...
                            if (multi_exchange_feed_)
                            {
                                ...
                                open_dtc_server::util::log("[SERVER] Removed subscription for " + symbol + " from all exchanges");
                            }
                        }
                    }
                    ...
                    open_dtc_server::util::log("[SERVER] Dynamic subscriptions updated: " + std::to_string(needed_symbols.size()) + " active symbols");
                }
            }
        }
